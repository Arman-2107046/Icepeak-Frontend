...
const WovenOuterwear = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/products`)
      .then((res) => res.json())
      .then((data: Product[]) => {
        const wovenOuterwearProducts = data.filter((product) =>
          product.category.toLowerCase().includes("woven-outerwear")
        );
        setProducts(wovenOuterwearProducts);
        setLoading(false);
      })
      .catch((err) => {
        console.error(err);
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{
          backgroundImage: "url('/productImages/outer-hero.jpg')",
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-center h-full px-12 text-left">
          <h1 className="max-w-3xl font-serif text-3xl font-semibold text-white drop-shadow-lg">
            Explore Our Premium Woven Outerwear Collection
          </h1>
          <p className="max-w-2xl mt-2 text-lg font-light text-white drop-shadow-md">
            Stylish and durable outerwear crafted for comfort, protection, and
            everyday elegance.
          </p>
        </div>
      </section>
