...
const WovenDenim = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/products`)
      .then((res) => res.json())
      .then((data: Product[]) => {
        const wovenDenimProducts = data.filter((product) =>
          product.category.toLowerCase().includes("woven-denim")
        );
        setProducts(wovenDenimProducts);
        setLoading(false);
      })
      .catch((err) => {
        console.error(err);
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{
          backgroundImage: "url('/productImages/denim-hero.jpg')",
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-center h-full px-12 text-left">
          <h1 className="max-w-3xl font-serif text-3xl font-semibold text-white drop-shadow-lg">
            Explore Our Premium Woven & Denim Collection
          </h1>
          <p className="max-w-2xl mt-2 text-lg font-light text-white drop-shadow-md">
            Durable, stylish, and crafted with precision — our woven and denim
            products redefine everyday wear.
          </p>
        </div>
      </section>
      ...
      <section className="px-6 mb-12 md:px-12 lg:px-24">
        <h2 className="mb-8 font-serif text-3xl font-bold text-gray-800">
          Our Woven & Denim Products
        </h2>
        {loading ? (
          <p className="text-gray-500">Loading products...</p>
        ) : products.length === 0 ? (
          <p className="text-gray-500">No woven & denim products available.</p>
        ) : (
