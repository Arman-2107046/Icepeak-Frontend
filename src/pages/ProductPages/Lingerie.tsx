...
const Lingerie = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/products`)
      .then((res) => res.json())
      .then((data: Product[]) => {
        const lingerieProducts = data.filter((product) =>
          product.category.toLowerCase().includes("lingerie")
        );
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{
          backgroundImage: "url('/productImages/lingerie-hero.jpg')",
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-center h-full px-12 text-left">
          <h1 className="max-w-3xl font-serif text-3xl font-semibold text-white drop-shadow-lg">
            Explore Our Premium Lingerie Collection
          </h1>
          <p className="max-w-2xl mt-2 text-lg font-light text-white drop-shadow-md">
            Elegant, comfortable, and stylish lingerie designed to inspire
      ...
      <section className="px-6 mb-12 md:px-12 lg:px-24">
        ...
        {loading ? (
          ...
          <p className="text-gray-500">No lingerie products available.</p>
        ) : (
