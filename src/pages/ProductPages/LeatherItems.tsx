...
const LeatherItems = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/products`)
      .then((res) => res.json())
      .then((data: Product[]) => {
        const leatherProducts = data.filter((product) =>
          product.category.toLowerCase().includes("leather")
        );
        setProducts(leatherProducts);
        setLoading(false);
      })
      .catch((err) => {
        console.error(err);
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{
          backgroundImage: "url('/productImages/leather-hero.jpg')",
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-center h-full px-12 text-left">
          ...
          <p className="max-w-2xl mt-2 text-lg font-light text-white drop-shadow-md">
            Durable, stylish, and timeless leather items crafted with precision
            and care.
          </p>
        </div>
      </section>
      ...
      <section className="px-6 mb-12 md:px-12 lg:px-24">
        ...
        {loading ? (
          ...
          <p className="text-gray-500">No leather products available.</p>
        ) : (
