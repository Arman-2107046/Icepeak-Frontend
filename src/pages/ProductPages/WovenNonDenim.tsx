...
const WovenNonDenim = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/products`)
      .then((res) => res.json())
      .then((data: Product[]) => {
        const wovenNonDenimProducts = data.filter((product) =>
          product.category.toLowerCase().includes("woven-non-denim")
        );
        setProducts(wovenNonDenimProducts);
        setLoading(false);
      })
      .catch((err) => {
        console.error(err);
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{
          backgroundImage: "url('/productImages/woven-hero.jpg')",
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-center h-full px-12 text-left">
          <h1 className="max-w-3xl font-serif text-3xl font-semibold text-white drop-shadow-lg">
            Explore Our Premium Woven Non-Denim Collection
          </h1>
          <p className="max-w-2xl mt-2 text-lg font-light text-white drop-shadow-md">
            Stylish, high-quality woven products crafted for comfort and
            versatility.
          </p>
        </div>
      </section>
      ...
      <section className="px-6 mb-12 md:px-12 lg:px-24">
        <h2 className="mb-8 font-serif text-3xl font-bold text-gray-800">
          Our Woven Non-Denim Products
        </h2>
        {loading ? (
          <p className="text-gray-500">Loading products...</p>
        ) : products.length === 0 ? (
          <p className="text-gray-500">No woven non-denim products available.</p>
        ) : (
